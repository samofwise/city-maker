...
function spiralSites(
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    return {
      x: Math.max(12, Math.min(cx * 2 - 12, cx + Math.cos(a) * Math.min(r, maxR * 0.85))),
      y: Math.max(12, Math.min(cy * 2 - 12, cy + Math.sin(a) * Math.min(r, maxR * 0.85))),
    };
  });
}
...
export function generateCity(
  ...
  const midIds     = new Set(sorted.slice(innerCount + 1, innerCount + midCount + 1).map((c) => c.id));
  // Outer cells: farm / forest based on random
  const outerIds = sorted.slice(innerCount + midCount + 1);

  // Furthest cells = water (border)
  const waterCutoff = dist(sorted[sorted.length - 1].siteX, sorted[sorted.length - 1].siteY, cx, cy) * 0.82;
  const waterIds = new Set(outerIds.filter((c) => (cellDist.get(c.id) ?? 0) > waterCutoff).map((c) => c.id));

  // ── 3. Assign terrain ─────────────────────────────────────────────────────
  mesh.cells.get(plazaId)!.data = { terrain: "market", zoneId: "plaza", density: 0.3, withinWalls: true };
  ...
  for (const c of outerIds) {
    if (waterIds.has(c.id)) {
      mesh.cells.get(c.id)!.data = { terrain: "water", zoneId: "ocean", density: 0, withinWalls: false };
    } else {
      const r = rng();
      const terrain = r < 0.45 ? "farm" : r < 0.75 ? "forest" : "empty";
      mesh.cells.get(c.id)!.data = {
        terrain,
        zoneId: terrain === "farm" ? "farmland" : terrain === "forest" ? "forest-zone" : null,
        density: 0.3 + rng() * 0.5,
        withinWalls: false,
      };
    }
  ...
  const wallRadius = sorted[innerCount + midCount].siteX !== undefined
  ...
  for (const angle of roadAngles) {
    ...
    for (let step = 0; step < 20; step++) {
      ...
      for (const eid of adjEdges) {
        ...
        const dx = nc.siteX - cx, dy = nc.siteY - cy;
      ...
      if (bestEdge.data.feature !== "wall") {
        ...
        [va, vb].forEach((v) => { v.data = { feature: "gate" }; });
        bestEdge.data = { feature: "road", width: 5 };
      }
  ...
  let riverCell: CellId | null = sorted.find((c) => c.siteY < height * 0.2)?.id ?? null;

  if (riverCell) {
    const riverVisited = new Set<CellId>();
    for (let step = 0; step < 25 && riverCell; step++) {
      riverVisited.add(riverCell);
      riverCells.add(riverCell);
      const adjEdges = mesh.index.cellEdges.get(riverCell) ?? [];
      let bestEid: EdgeId | null = null;
      let bestScore = -Infinity;
      ...
      for (const eid of adjEdges) {
        ...
        const next = adjC.find((c) => c !== riverCell && !riverVisited.has(c));
        if (!next) continue;
        const nc = mesh.cells.get(next)!;
        const dx = nc.siteX - cx, dy = nc.siteY - cy;
        const score = Math.cos(riverAngle) * dx + Math.sin(riverAngle) * dy
      ...
      riverCell = adjC.find((c) => c !== riverCell && !riverVisited.has(c)) ?? null;
    }
  }
  ...
  for (const [, v] of mesh.vertices) {
    ...
    const hasRoad  = vEdges.some((eid) => mesh.edges.get(eid)!.data.feature === "road");
    const hasRiver = vEdges.some((eid) => mesh.edges.get(eid)!.data.feature === "river");
    const hasWall  = vEdges.some((eid) => mesh.edges.get(eid)!.data.feature === "wall");
    ...
    if (hasRiver && hasRoad) {
      ...
      const wallEdgeCount = vEdges.filter((eid) => mesh.edges.get(eid)!.data.feature === "wall").length;
  ...
  const addZone = (id: string, name: string, type: Zone["type"], density: number) => {
